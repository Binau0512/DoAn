...
class OrderModel extends BaseModel
{
    ...
    {
        $sql = "SELECT sum(order_detail.price) as total_price FROM `order` inner join `order_detail` on `order`.`id` = `order_detail`.`order_id` WHERE `order`.`status` = 0";
        $query = $this->_query($sql);
        return mysqli_fetch_assoc($query);
    }
    ...
    {
        $sql = "SELECT `order`.*, SUM(price) AS total, SUM(quantity) AS quantity FROM `order_detail` INNER JOIN `order` ON `order_detail`.`order_id` = `order`.`id` where `order`.`id` = $id";
        $query = $this->_query($sql);
