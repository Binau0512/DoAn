...
class ProductModel extends BaseModel
{
    ...
    {
        $sql = "SELECT sum(order_detail.quantity) as purchased, RANK() OVER (ORDER BY purchased DESC) rank,  product.* , category.name as category_name from order_detail, product, category where product.id = order_detail.product_id and product.category_id = category.id GROUP BY product.id order by purchased DESC LIMIT 5";
        return $this->getByQuery($sql);
    }
    ...
    {
        $sql = "SELECT sum(order_detail.quantity) as purchased, RANK() OVER (ORDER BY purchased DESC) rank,  product.* , category.name as category_name from order_detail, product, category where product.id = order_detail.product_id and product.category_id = category.id GROUP BY product.id order by purchased DESC LIMIT 1";
    ...
    public function updateQuantity($id, $quan)
    {
        ...
        $query = $this->_query($sql);
        $afterBuy = mysqli_fetch_assoc($query)['quantity'] - $quan;
        $data = [
            'quantity' => $afterBuy
        ];
